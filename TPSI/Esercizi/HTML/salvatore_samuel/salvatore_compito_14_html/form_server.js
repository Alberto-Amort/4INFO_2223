...
app.post('/aggiungi', function(req,res){
    ...
    if (req.body.eta_studente > 19 && req.body.eta_studente < 31) {
        console.log(`OK: id ${req.body.eta_studente} valido`);
        res.send(`<h1>OK studente aggiunto con età [${req.body.eta_studente}]</h1>`);
    } else {
        console.log(`ERROR: età ${req.body.eta_studente} non valida`);
        res.send(`<script>setTimeout(function(){ window.location.href = "/"; }, 3000);</script>
                  <h1>ERROR: non posso aggiungere lo studente con età [${req.body.id_studente}]</h1>`);
    }
});
